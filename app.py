...
def chat(
    ...
    try:
        ...
        print(f"Response data received: {type(response_data)}")
        
        # Check if response_data has the expected structure
        if not isinstance(response_data, dict):
            print(f"ERROR: Expected dict, got {type(response_data)}: {response_data}")
            raise ValueError(f"Invalid response format from process_query: {type(response_data)}")
        
        if 'messages' not in response_data:
            print(f"ERROR: No 'messages' key in response_data: {response_data.keys()}")
            raise ValueError("Missing 'messages' in response data")
        
        if not response_data['messages']:
            print("ERROR: Empty messages list")
            raise ValueError("Empty messages list in response data")
        ...
        # If it's the brain tumor detection agent, check for output image
        elif response_data.get("agent_name") == "BRAIN_TUMOR_AGENT, HUMAN_VALIDATION":
            detection_path = os.path.join(BRAIN_TUMOR_OUTPUT, "detection_result.png")
            if os.path.exists(detection_path):
                result["result_image"] = f"/uploads/brain_tumor_output/detection_result.png"
            else:
                print("Brain Tumor Detection Output path does not exist.")
...
async def upload_image(
    ...
    try:
        ...
        # If it's the brain tumor detection agent, check for output image
        elif response_data["agent_name"] == "BRAIN_TUMOR_AGENT, HUMAN_VALIDATION":
            detection_path = os.path.join(BRAIN_TUMOR_OUTPUT, "detection_result.png")
            if os.path.exists(detection_path):
                result["result_image"] = f"/uploads/brain_tumor_output/detection_result.png"
            else:
