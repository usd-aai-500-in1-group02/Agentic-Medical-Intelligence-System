...
def create_agent_graph():
    ...
    def route_to_agent(state: AgentState) -> Dict:
        ...
        # Direct routing for medical images to avoid unnecessary OpenAI calls
        if has_image and image_type:
            image_type_upper = image_type.upper()
            
            # Route medical images directly to their respective local models
            if "CHEST X-RAY" in image_type_upper or "CHEST_XRAY" in image_type_upper:
                print(f"Direct routing: CHEST X-RAY image detected, routing to CHEST_XRAY_AGENT")
                updated_state = {
                    **state,
                ...
                print(f"Direct routing: Brain MRI image detected, routing to BRAIN_TUMOR_AGENT")
                updated_state = {
                    **state,
            ...
            elif "SKIN LESION" in image_type_upper or "SKIN_LESION" in image_type_upper:
                print(f"Direct routing: Skin lesion image detected, routing to SKIN_LESION_AGENT")
                updated_state = {
                    **state,
                ...
                print(f"Direct routing: Non-medical or other image type, routing to CONVERSATION_AGENT")
                updated_state = {
                    **state,
        ...
        # For non-image queries or unclassified images, use the OpenAI decision chain
        print(f"Using OpenAI decision chain for routing (no image or unclassified image)")
    ...
    def run_brain_tumor_agent(state: AgentState) -> AgentState:
        ...
        # classify brain MRI for tumor detection
        predicted_class = AgentConfig.image_analyzer.classify_brain_tumor(image_path)
        
        # generate visualization for UI display
        visualization_success = AgentConfig.image_analyzer.visualize_brain_tumor_detection(image_path)
        
        if not visualization_success:
            print("Warning: Failed to generate brain tumor detection visualization")

        if predicted_class == "tumor_detected":
            response = AIMessage(content="The analysis of the uploaded brain MRI image indicates **TUMOR DETECTED**. A potential brain tumor has been identified in the scan.")
        elif predicted_class == "no_tumor":
            response = AIMessage(content="The analysis of the uploaded brain MRI image indicates **NO TUMOR DETECTED**. The scan appears to be **NORMAL**.")
        else:
            response = AIMessage(content="The uploaded image could not be analyzed properly. Please ensure the image is a clear brain MRI scan.")
