...
class AgentDecisoinConfig:
    def __init__(self):
        self.llm = ChatOpenAI(
            model = os.getenv("OPENAI_MODEL", "gpt-4"),  # OpenAI model name
...
class RAGConfig:
    def __init__(self):
        ...
        self.collection_name = "medirag"  # Ensure a valid name
        self.chunk_size = 1024  # Modify based on documents and performance
        self.chunk_overlap = 200  # Modify based on documents and performance
        # self.embedding_model = "text-embedding-3-large"
        # Initialize Azure OpenAI Embeddings
        self.embedding_model = OpenAIEmbeddings(
            model = os.getenv("OPENAI_EMBEDDING_MODEL", "text-embedding-3-small"),  # OpenAI embedding model
            openai_api_key = os.getenv("OPENAI_API_KEY")  # OpenAI API key
        )
        self.llm = ChatOpenAI(
            model = os.getenv("OPENAI_MODEL", "gpt-4"),  # OpenAI model name
...
class MedicalCVConfig:
    def __init__(self):
        self.brain_tumor_model_path = "./agents/image_analysis_agent/brain_tumor_agent/models/brain_tumour_od.pt"
        self.chest_xray_model_path = "./agents/image_analysis_agent/chest_xray_agent/models/covid_chest_xray_model.pth"
        self.skin_lesion_model_path = "./agents/image_analysis_agent/skin_lesion_agent/models/checkpointN25_.pth.tar"
        self.skin_lesion_segmentation_output_path = "./uploads/skin_lesion_output/segmentation_plot.png"
        self.brain_tumor_detection_output_path = "./uploads/brain_tumor_output/detection_result.png"
        self.llm = ChatOpenAI(
            model = os.getenv("OPENAI_VISION_MODEL", "gpt-4o"),  # Vision-capable model for image analysis
            openai_api_key = os.getenv("OPENAI_API_KEY"),  # OpenAI API key
